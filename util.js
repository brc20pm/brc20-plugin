...
const util = {
    ...
    获取当前工作空间下可编译的js文件
    */
    getPorjectFiles(rootPath) {
        rootPath = rootPath.substring(1, rootPath.length)
        let projects = fs.readdirSync(rootPath);
        let fileList = []
        projects.forEach(p => {
            //如果不是js文件
            let isJSFile = p.endsWith('.js');
            if (!isJSFile) {
                return
            }
            let lPath = path.join(rootPath, p)
            let stat = fs.lstatSync(lPath)
            if (stat.isFile()) {
                fileList.push({
                    name: p,
                    path: lPath
    ...
    */
    ...
    showError: function (info) {
        let context = typeof info === 'object' ? JSON.stringify(info, null, 2) : info
